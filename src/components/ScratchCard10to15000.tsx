...
const PRIZES = {
  "perdeu": { 
    value: 0, 
    label: "VOCÊ PERDEU", 
    image: "/lovable-uploads/a1d87b84-ddef-400a-9703-8d699aa2721c.png"
  },
  "2-reais": { 
    value: 2, 
    label: "VOCÊ GANHOU R$ 2,00", 
    image: "/lovable-uploads/bf9c208d-2823-4813-9327-742415b913c5.png"
  },
  "20-reais": { 
    value: 20, 
    label: "VOCÊ GANHOU R$ 20,00", 
    image: "/lovable-uploads/d21b6d72-b87b-4aab-9ed9-941c3d838823.png"
  },
  "50-reais": { 
    value: 50, 
    label: "VOCÊ GANHOU R$ 50,00", 
    image: "/lovable-uploads/c5bdcb11-72a4-4f5a-91ba-52829ec6b111.png"
  }
};

const ScratchCard10to15000 = () => {
  ...
  const getPrizeForCard = (cardNumber: number): string => {
    ...
    if (cycle >= 1 && cycle <= 5) return "perdeu"; // Cartelas 1-5: não ganha nada
    if (cycle === 6) return "20-reais"; // Cartela 6: ganha 20 reais
    if (cycle >= 7 && cycle <= 10) return "perdeu"; // Cartelas 7-10: não ganha nada
    if (cycle >= 11 && cycle <= 13) return "2-reais"; // Cartelas 11-13: ganha 2 reais
    if (cycle >= 14 && cycle <= 19) return "perdeu"; // Cartelas 14-19: não ganha nada
    if (cycle === 20) return "50-reais"; // Cartela 20: ganha 50 reais
    if (cycle >= 21 && cycle <= 27) return "perdeu"; // Cartelas 21-27: não ganha nada
    if (cycle >= 28 && cycle <= 30) return "2-reais"; // Cartelas 28-30: ganha 2 reais
    if (cycle >= 31 && cycle <= 35) return "perdeu"; // Cartelas 31-35: não ganha nada
    if (cycle === 36) return "20-reais"; // Cartela 36: ganha 20 reais
    if (cycle >= 37 && cycle <= 40) return "perdeu"; // Cartelas 37-40: não ganha nada
    if (cycle >= 41 && cycle <= 43) return "2-reais"; // Cartelas 41-43: ganha 2 reais
    if (cycle >= 44 && cycle <= 48) return "perdeu"; // Cartelas 44-48: não ganha nada
    if (cycle === 49) return "50-reais"; // Cartela 49: ganha 50 reais
    if (cycle === 50) return "perdeu"; // Cartela 50: não ganha nada
  ...
  const buyCard = async () => {
    const price = 10;
    const requestedGame = 'Raspadinha 10 Reais';
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-casino-header/50 border-b border-border p-4 flex items-center justify-between">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate('/raspadinha-10-para-15000/regras')}
            className="h-8 w-8"
          >
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex gap-4">
            <Button 
              onClick={buyCard}
              disabled={balance < 10 || (cardGenerated && !isRevealed)}
...
export default ScratchCard10to15000;