-- Case-insensitive partner email matching in coupon functions
...
BEGIN
  ...
  -- If no deposit required, add to bonus balance directly (unlocked for playing but still subject to 1x rollover)
  UPDATE public.profiles
  SET bonus_balance = COALESCE(bonus_balance, 0) + v_bonus_amount,
      bonus_game_restriction = v_game_restriction,
      bonus_locked = false,
  ...
  IF v_partner_email IS NOT NULL AND v_partner_email <> '' AND NOT v_requires_deposit AND v_partner_commission > 0 THEN
    SELECT id INTO v_partner_user_id
    FROM public.profiles
    WHERE lower(email) = lower(v_partner_email)
    LIMIT 1;
...
-- Unlock bonus when deposit made: case-insensitive partner email
