-- Drop ALL policies on referrals that reference referral_id
...
DROP POLICY IF EXISTS "referrals_service_role_insert" ON public.referrals;
DROP POLICY IF EXISTS "referrals_service_role_update" ON public.referrals;

-- Drop old function signature
DROP FUNCTION IF EXISTS public.process_referral_bonus(uuid, numeric);

-- Ensure unique referral codes and automatic generation in format bdc###
ALTER TABLE public.profiles ALTER COLUMN referral_id TYPE text;
...
WHERE referral_id IS NULL OR length(referral_id) = 0;

-- Recreate all RLS policies for referrals
CREATE POLICY "Users can view their own referrals" 
ON public.referrals 
FOR SELECT
USING ((referrer_id IN (SELECT profiles.referral_id FROM profiles WHERE profiles.id = auth.uid())) OR (referred_user_id = auth.uid()));
...
ON public.referrals 
FOR SELECT
USING ((referred_user_id = auth.uid()) OR (referrer_id IN (SELECT profiles.referral_id FROM profiles WHERE profiles.id = auth.uid())));

CREATE POLICY "referrals_service_role_insert" 
ON public.referrals 
FOR INSERT
WITH CHECK (true);

CREATE POLICY "referrals_service_role_update" 
ON public.referrals 
FOR UPDATE
USING (true)
WITH CHECK (true);

-- Ensure profiles and credentials are created on signup
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
AFTER INSERT ON auth.users
FOR EACH ROW EXECUTE PROCEDURE public.handle_new_user_signup();

-- Update referral bonus logic with new param names
